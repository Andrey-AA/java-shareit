...
class BookingRepositoryTests {
    ...
    void findBookingsByUser() {
        ...
        List<Booking> bookingsByUser = bookingRepository.findAllByBookerIdOrderByStartDesc(user.getId(), Pageable.ofSize(10)).getContent();
        assertThat((long) bookingRepository.findAllByBookerIdOrderByStartDesc(user2.getId(), Pageable.ofSize(10)).getContent().size(), equalTo(1L));
    }
